...
The page is a single reading column: bounded queue offer, eligible work, an illustrative paid pilot, three ordered engagement steps, operator context and contact. The offer names agreed systems, working hours, capacity and an escalation owner; Jon reviews agent-prepared changes through the customer's tooling and approval process. A discrete project remains available, while any month-to-month work follows a successful pilot and stays bounded. Direct email is the primary action; the optional structured brief lives in a native disclosure. Form fields pair above 640px. Motion is a brief, already-visible positional entrance with reduced-motion support.
